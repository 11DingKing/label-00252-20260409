<template>
  <div class="dashboard">
    ...
    <el-row :gutter="20" class="stats-row">
      <el-col :xs="24" :sm="12" :lg="6">
        <div class="stat-card success">
          <div class="stat-content">
            <div class="stat-value">{{ formatPower(summary.total_generation) }}</div>
      ...
      <el-col :xs="24" :sm="12" :lg="6">
        <div class="stat-card" :class="summary.grid_power > 0 ? 'info' : 'success'">
          <div class="stat-content">
            <div class="stat-value">{{ formatPower(Math.abs(summary.grid_power || 0)) }}</div>
            <div class="stat-label">{{ summary.grid_power > 0 ? '购电功率' : '售电功率' }} (kW)</div>
            <div class="stat-trend" :class="summary.grid_power > 0 ? '' : 'up'">
              <el-icon v-if="summary.grid_power <= 0"><Top /></el-icon>
              <span>{{ summary.grid_mode === 'grid_connected' ? '并网运行' : '离网运行' }}</span>
            </div>
          </div>
          <div class="stat-icon">
      ...
      <el-col :xs="24" :sm="12" :lg="6">
        <div class="stat-card" :class="getBatterySocClass()">
          <div class="stat-content">
            <div class="stat-value">{{ formatPercent(summary.battery_soc) }}%</div>
          ...
          <div class="battery-bar">
            <div class="battery-level" :style="{ width: formatPercent(summary.battery_soc) + '%' }"></div>
          </div>
        </div>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="20">
      <el-col :xs="24" :lg="16">
        <div class="card">
          ...
          <v-chart :option="powerChartOption" style="height: 320px" autoresize />
        </div>
      </el-col>
      <el-col :xs="24" :lg="8">
        <div class="card">
    ...
    <el-row :gutter="20">
      <el-col :xs="24" :md="8">
        <div class="card system-card">
          <div class="card-header">
            <h3>光伏系统</h3>
            <span class="status-indicator" :class="summary.pv_power > 0 ? 'online' : 'offline'">
          ...
          <div class="system-stats">
            ...
            <div class="stat-row">
              <span>利用率</span>
              <span>{{ ((summary.pv_power || 0) / 500 * 100).toFixed(1) }}%</span>
            </div>
          </div>
        </div>
      </el-col>
      <el-col :xs="24" :md="8">
        <div class="card system-card">
          <div class="card-header">
            <h3>风力发电</h3>
            <span class="status-indicator" :class="summary.wind_power > 0 ? 'online' : 'offline'">
              {{ summary.wind_power > 0 ? '发电中' : '停机' }}
            </span>
          </div>
          <div class="system-gauge">
            <v-chart :option="windGaugeOption" style="height: 160px" autoresize />
            <div class="gauge-info">
              <div class="gauge-value">{{ formatPower(summary.wind_power) }}</div>
              <div class="gauge-unit">kW</div>
            </div>
          </div>
          <div class="system-stats">
            ...
            <div class="stat-row">
              <span>利用率</span>
              <span>{{ ((summary.wind_power || 0) / 300 * 100).toFixed(1) }}%</span>
            </div>
          </div>
        </div>
      </el-col>
      <el-col :xs="24" :md="8">
        <div class="card system-card">
          ...
          <div class="system-gauge">
            <v-chart :option="batteryGaugeOption" style="height: 160px" autoresize />
            <div class="gauge-info">
              <div class="gauge-value">{{ formatPercent(summary.battery_soc) }}</div>
    ...
    <el-row :gutter="20">
      <el-col :span="24">
        <div class="card grid-status-card">
          <div class="card-header">
            <h3>电网状态</h3>
            <el-tag :type="summary.grid_mode === 'grid_connected' ? 'success' : 'warning'" effect="dark">
          ...
          <el-row :gutter="40">
            <el-col :xs="12" :md="6">
              <div class="grid-metric">
                ...
                <div class="metric-info">
                  <div class="metric-value">{{ (summary.grid_voltage || 380).toFixed(1) }}</div>
            ...
            <el-col :xs="12" :md="6">
              <div class="grid-metric">
                ...
                <div class="metric-info">
                  <div class="metric-value">{{ (summary.grid_frequency || 50).toFixed(2) }}</div>
            ...
            <el-col :xs="12" :md="6">
              <div class="grid-metric">
                ...
                <div class="metric-info">
                  <div class="metric-value">{{ formatPercent(summary.renewable_ratio) }}</div>
            ...
            <el-col :xs="12" :md="6">
              <div class="grid-metric">
                ...
                <div class="metric-info">
                  <div class="metric-value">{{ formatPercent(summary.self_sufficiency) }}</div>
...
const loadCount = computed(() => Object.values(realtimeStore.loadData).filter(l => l.is_on).length)
const renewablePercent = computed(() => formatPercent(summary.value.renewable_ratio))
...
function getVoltageStatus() {
  const v = summary.value.grid_voltage || 380
  return Math.abs(v - 380) / 380 <= 0.05 ? 'normal' : 'warning'
}

function getFrequencyStatus() {
  const f = summary.value.grid_frequency || 50
  return Math.abs(f - 50) <= 0.2 ? 'normal' : 'warning'
}
...
const powerChartOption = computed(() => ({
  ...
  series: [
    { name: '发电', type: 'line', smooth: true, symbol: 'none', data: history.value.map(h => h.total_generation || 0), 
    ...
    { name: '负载', type: 'line', smooth: true, symbol: 'none', data: history.value.map(h => h.total_load || 0),
      lineStyle: { width: 2, color: chartColors.load },
      areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(253,203,110,0.3)' }, { offset: 1, color: 'rgba(253,203,110,0)' }] } }
    },
    { name: '电网', type: 'line', smooth: true, symbol: 'none', data: history.value.map(h => h.grid_power || 0),
      lineStyle: { width: 2, color: chartColors.grid }
    },
    { name: '储能', type: 'line', smooth: true, symbol: 'none', data: history.value.map(h => h.battery_power || 0),
...
const pieChartOption = computed(() => ({
  ...
  series: [{
    ...
    data: [
      { value: summary.value.pv_power || 0, name: '光伏', itemStyle: { color: '#00D4AA' } },
      { value: summary.value.wind_power || 0, name: '风电', itemStyle: { color: '#74B9FF' } },
      { value: Math.max(0, summary.value.grid_power || 0), name: '电网', itemStyle: { color: '#A29BFE' } }
    ]
  }]
}))
...
const pvGaugeOption = computed(() => ({
  ...
  series: [{
    ...
    progress: { ...gaugeBaseOption.series[0].progress, itemStyle: { color: '#00D4AA' } }
  }]
}))
...
const windGaugeOption = computed(() => ({
  ...
  series: [{
    ...
    progress: { ...gaugeBaseOption.series[0].progress, itemStyle: { color: '#74B9FF' } }
  }]
}))
...
const batteryGaugeOption = computed(() => ({
  ...
  series: [{
    ...
    progress: { ...gaugeBaseOption.series[0].progress, itemStyle: { color: '#FDCB6E' } }
  }]
}))
</script>
...
.chart-legend {
  ...
  .legend-item {
    ...
    &.generation i { background: #00D4AA; }
    &.load i { background: #FDCB6E; }
    &.grid i { background: #74B9FF; }
    &.battery i { background: #A29BFE; border-style: dashed; }
  }
}
...
.grid-status-card {
  .grid-metric {
    ...
    .metric-icon {
      ...
      &.voltage { background: rgba(116,185,255,0.15); color: #74B9FF; }
      &.frequency { background: rgba(253,203,110,0.15); color: #FDCB6E; }
      &.renewable { background: rgba(0,212,170,0.15); color: #00D4AA; }
      &.self { background: rgba(162,155,254,0.15); color: #A29BFE; }
    }
